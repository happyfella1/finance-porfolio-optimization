...
def getConstraints(k, short, df):
    ...
    # else:
    totalMeans = df.mean().values
    monthlyMeans = pd.groupby(df,by=[df.index.month]).mean()
    # construct deviation matrix
    deviationDf = monthlyMeans.apply(lambda row: row - totalMeans,axis=1)
    deviationMatrix = matrix(np.matrix(deviationDf))
    dev = sparse([[deviationMatrix,-deviationMatrix]])
    iden = sparse([[matrix(-np.identity(k)),matrix(-np.identity(k))]])

    sumArray = np.ones((k,), dtype=np.int) + np.zeros((k,), dtype=np.int)
    constraintMatrix = sparse([[dev,iden]])
